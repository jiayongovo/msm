...
template <class bucket_t, class affine_t, class scalar_t> struct Context {
  // pippenger
  pipp_t pipp;
  // MSMConfig
...
template <class bucket_t, class affine_t, class scalar_t> struct RustContext {
...
mult_pippenger_faster_init(RustContext<bucket_t, affine_t, scalar_t> *context,
  ...
  try {
    ...
    ctx->jy_d_scalar_tuples_sn =
        ctx->pipp.allocate_jy_d_scalar_tuple(ctx->config);
    ctx->jy_d_point_idx_sn = ctx->pipp.allocate_jy_d_point_idx(ctx->config);
    ctx->jy_d_scalar_tuples_out_sn =
        ctx->pipp.allocate_jy_d_scalar_tuple_out(ctx->config);
  ...
  try {
    ...
    for (; work < (int)batches; work++) {
      ...
      batch_pool.spawn([&]() {
        ...
        // scalar point
        uint32_t *jy_d_scalar_tuple =
            ctx->pipp.jy_d_scalar_tuple_ptrs[ctx->jy_d_scalar_tuples_sn];
        uint32_t *jy_d_scalar_tuple_out =
            ctx->pipp.jy_d_scalar_tuple_ptrs[ctx->jy_d_scalar_tuples_out_sn];
        uint32_t *jy_d_point_idx =
            ctx->pipp.jy_d_point_idx_ptrs[ctx->jy_d_point_idx_sn];
        uint32_t *jy_d_point_idx_out =
            ctx->pipp.jy_d_point_idx_ptrs[ctx->jy_d_point_idx_out_sn];
        ...
        cub::DeviceRadixSort::SortPairs(
            d_temp, temp_sort_size, jy_d_scalar_tuple, jy_d_scalar_tuple_out,
        ...
        for (size_t k = 0; k < NWINS; k++) {
          ...
          cub::DeviceRadixSort::SortPairs(
              d_cub_sort, temp_sort_size, jy_d_scalar_tuple + ptr,
              jy_d_scalar_tuple_out + ptr, jy_d_point_idx + ptr,
              jy_d_point_idx_out + ptr, nscalars, 0, 31, stream);
        }

        // accumulate parts of the buckets into static buffers.
        // 预计算点
        ctx->pipp.launch_bucket_acc(
            ctx->config, ctx->jy_d_scalar_tuples_out_sn,
            ctx->jy_d_point_idx_out_sn, ctx->d_pre_points_sn, ctx->d_buckets_sn,
