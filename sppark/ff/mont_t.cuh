...
public:
    ...
    public:
	    ...
	    inline wide_t& operator-=(const wide_t& b) {
            size_t i;
            const size_t nn = 2*n;
            uint32_t tmp[nn], borrow;
            ...
            asm("add.cc.u32 %0, %1, 0;" : "=r"(tmp[0]) : "r"(even[0]));
            for (i = 1; i < n; i++)
                asm("addc.cc.u32 %0, %1, 0;" : "=r"(tmp[i]) : "r"(even[i]));
            for (i = n; i < nn-1; i++)
                asm("addc.cc.u32 %0, %1, %2;" : "=r"(tmp[i]) : "r"(even[i]), "r"(MOD[i-n]));
            asm("addc.u32 %0, %1, %2;" : "=r"(tmp[i]) : "r"(even[i]), "r"(MOD[n-1]));
        ...
        friend inline wide_t operator-(wide_t a, const wide_t& b) {
            return a -= b;
        }
...
public:
    ...
    friend inline mont_t normalize(const wide_t& a) {
        union { wide_t w; mont_t s[2]; } ret = { a };
        ret.s[0].mul_by_1();
        return ret.s[0] += ret.s[1];
    }
    ...
    inline mont_t const inv() {
        uint32_t inv_exp[n];
        int i;

        asm("sub.cc.u32 %0, %1, 2;" : "=r"(inv_exp[0]) : "r"(MOD[0]));
        for (i = 1; i < n - 1; i++) {
            asm("subc.cc.u32 %0, %1, 0;" : "=r"(inv_exp[i]) : "r"(MOD[i]));
        }
        asm("subc.u32 %0, %1, 0;" : "=r"(inv_exp[n - 1]) : "r"(MOD[n - 1]));

	    mont_t ret(ONE);
	    mont_t inv_base = *this;
...
//            while (inv_exp[i] > 0) {
    ...
    inline bool is_one() const {
        size_t i;
        uint32_t is_same = 0, is_one = 0;

        for (i = 0; i < n; i++) {
            asm("xor.b32 %0, %1, %2;" : "+r"(is_same) : "r"(even[i]), "r"(ONE[i]));
            asm("or.b32 %0, %0, %1;" : "+r"(is_one) : "r"(is_same));
        }
        asm("set.eq.u32.u32 %0, %0, 0;" : "+r"(is_one));
