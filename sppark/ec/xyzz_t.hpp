...
public:
    ...
    public:
        ...
        inline __device__ void set_inf()
        {
            inf[0] = 1;
        }
        ...
        inline __device__ void to_xyzz_t(xyzz_t &a)
        {
            a.X = this->X;
            a.Y = this->Y;
            if (this->is_inf())
        ...
        inline __device__ void uadd(const affine_inf_t &p2, bool subtract = false)
        {
#ifdef __CUDA_ARCH__
            affine_inf_t p31 = *this;
#else
            affine_inf_t &p31 = *this;
#endif
            if (p2.is_inf())
            ...
            else
            {
                ...
                R = p2.Y - p31.Y; /* R = Y2 - Y1 */

                P = p2.X - p31.X; /* P = X2 - X1*/
...
#undef PPP
                ...
                {                  /* X1==X2 && Y1==-Y2 */
                    p31.set_inf(); /* set |p3| to infinity */
                }
            }
#ifdef __CUDA_ARCH__
            *this = p31;
    ...
    inline __device__ xyzz_t(const affine_inf_t &a) { *this = a; }
    ...
    inline __device__ void to_affine(affine_t &a)
    {
        a.Y = ZZZ.inv();
        a.X = a.Y * ZZ; // 1/Z
        a.X = a.X ^ 2;  // 1/Z^2
    ...
    inline __device__ void to_affine_inf(affine_inf_t &a)
    {
    ...
    inline __device__ void xyzz_print()
    {
        ...
        for (int i = 11; i >= 0; i--)
        {
            printf("%u", *((uint32_t *)(&x) + i));
        }
        printf("\n");
        for (int i = 11; i >= 0; i--)
        {
            printf("%u", *((uint32_t *)(&y) + i));
        }
        printf("\n--------\n");
    }
